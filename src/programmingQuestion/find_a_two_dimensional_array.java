...
/**
 * @ClassName find_a_two_dimensional_array
 * @Descrcription 在一个二维数组中（每个一维数组的长度相同），每一行都按照从左到右递增的顺序排序，每一列都按照从上到下递增的顺序排序。请完成一个函数，输入这样的一个二维数组和一个整数，判断数组中是否含有该整数。
 * @Author zzc
 * @Date 2019/9/8 13:39
 * @Version 1.0
 */
public class find_a_two_dimensional_array {
    ...
    public static boolean Find(int target, int [][] array) {
        ...
        for (int k = 0; k < colNum; k++) {
            ...
            while (mid > start) {
                ...
                if (((end - start) / 2) == 0) {
                    if (array[end][k] == target | array[start][k] == target) {
                        return true;
                    } else if (array[end][k] < target) {
                        mid = end;
                        break;
                    } else {
                        mid = start;
            ...
            if (mid == rowNum && k == colNum) {
                return false;
            } else {
                if (mid != (rowNum-1)) {
                    if (array[mid][k] < target && array[mid + 1][k] > target) {
                        for (int j = k; j < colNum; j++) {
    ...
    public static void main(String[] args) {
        int[][] arr = { {1,2,4,6,8,10,11,13,15,17,20,22,24,27,30,32,35,36,38,39,42,44,46,47,48,49,51,54,55,56},
        ...
        if (res) {
            System.out.println("数组中有整数：" + target);
        } else {
            System.out.println("数组中没有整数：" + target);
        }
    }
}
